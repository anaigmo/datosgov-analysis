...
def generate_dqv_report(graph, shacl_report_graph):
    ...
    return(dqv_report.serialize(format='ttl'))

def load_to_fuseki(dqv_report_file):
    FUSEKI_URL = "http://fuseki:3030/ds"

    res = requests.post(
        f"{FUSEKI_URL}/data",
        headers={"Content-Type": "text/turtle"},
        data=dqv_report_file.encode("utf-8"),
        auth=("admin", "admin")
    )
...
if __name__ == "__main__":
    ...
    dqv_report_file = generate_dqv_report(graph, shacl_report_graph)
